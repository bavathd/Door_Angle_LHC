...
u8g2.sendBuffer();
  ...
  initX = atan(Oaccel.x/sqrt(sq(Oaccel.y)+sq(Oaccel.z)));
  initY = atan(Oaccel.y/sqrt(sq(Oaccel.x)+sq(Oaccel.z)));
  initZ = atan(sqrt(sq(Oaccel.x)+sq(Oaccel.y))/Oaccel.z);
...
digitalWrite(33, HIGH);
  ...
  Serial.println("");
  anglex = (atan(Oaccel.x/sqrt(sq(Oaccel.y)+sq(Oaccel.y))) - initX)* 57.29;
  angley = (atan(Oaccel.y/sqrt(sq(Oaccel.x)+sq(Oaccel.z))) - initY) * 57.29;
  anglez = (atan(sqrt(sq(Oaccel.y)+sq(Oaccel.y))/Oaccel.z) - initZ) * 57.29;
